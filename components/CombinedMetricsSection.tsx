import {
  hitTargetClass,
  metricsCombinedBlocks,
  type MetricsCombinedBlock,
  type MetricsCombinedTableBody,
...
export function CombinedMetricsSection({ body }: Props) {
  ...
  const blocks = metricsCombinedBlocks(body);

  return (
    <>
      {blocks.map((block, i) => (
        <div key={`${block.role}-${block.event_date}-${i}`} className="metrics-event-block">
          {block.label ? (
            <h3 className="metrics-event-label">{block.label}</h3>
          ) : null}
          <KeyMetricsCombinedBlockTable block={block} />
        </div>
      ))}
      {footerBits.length > 0 ? (
...
function KeyMetricsCombinedBlockTable({ block }: { block: MetricsCombinedBlock }) {
  const showRerating = block.show_rerating ?? false;
  const showResults = block.show_results ?? false;
